		...
		while (!cellIsOpen) {
			...
			// For PvE gamemodes
				...
				if (turns % 2 == 1) {
					...
					bot_two.setPosition(gameBoard);
					x_position = bot_two.getXPosition();
					y_position = bot_two.getYPosition();
				}
			}
			// A catch for corrupted gamemode data
			else {
				std::cout << "Unrecognized gamemode..." << std::endl;
				std::cout << "Please restart the game." << std::endl;
				system("pause");
				exit(0);
			}
			// Checks to see is the selected cell is open on the board
...
int game::victory() {
	// Checks all possible winning board configurations for the basic gamemode
	if (gameMode != 2) {
	...
	// Checks all winning board configurations for the advanced gamemode
	else {
